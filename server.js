...
// FIX: Render (and most hosts) run the app behind a reverse proxy, which
// adds an X-Forwarded-For header. Without telling Express to trust that
// proxy, express-rate-limit can't correctly identify each visitor's real
// IP, and logs the ERR_ERL_UNEXPECTED_X_FORWARDED_FOR warning. Setting
// this to 1 means "trust exactly one hop of proxy" (Render's own edge),
// which is correct and avoids the security risk of the more permissive
// `trust proxy: true` setting (which would trust ANY proxy, including one
// a malicious client could spoof).
...
// Category Schema — the 4 homepage cards (Men / Women / Unisex / New In).
// "slug" is a fixed key (men/women/unisex/new) so the frontend always knows
// which card to render where, while title/subtitle/image stay editable.
const categorySchema = new mongoose.Schema({
  slug:     { type: String, unique: true, required: true }, // 'men' | 'women' | 'unisex' | 'new'
  title:    { type: String, default: '' },
  subtitle: { type: String, default: '' },
  image:    { type: cloudImageSchema, default: null },
}, { timestamps: true });
...
function normalizeProduct(p) {
  ...
  // FIX: .lean() queries return sizeStock as a plain JS object, not a
  // Mongoose Map — Object.fromEntries() throws on a plain object because
  // it isn't iterable, which was causing a 500 on every products route.
  // This now safely normalizes either shape (Map or plain object) to a
  // plain object for the JSON response.
  let sizeStock = {};
...
// ============================================================
// CATEGORIES — the 4 homepage cards (Men / Women / Unisex / New In)
// ============================================================

const CATEGORY_SLUGS = ['men', 'women', 'unisex', 'new'];
const CATEGORY_DEFAULTS = {
  men:    { title: 'MEN',    subtitle: 'Essentials & Drops' },
  women:  { title: 'WOMEN',  subtitle: 'Fresh Styles' },
  unisex: { title: 'UNISEX', subtitle: 'For Everyone' },
  new:    { title: 'NEW IN', subtitle: 'Latest Drops' },
};

function normalizeCategory(c) {
  const image = c.image && typeof c.image === 'string' ? { url: c.image, publicId: null } : c.image;
  return { slug: c.slug, title: c.title, subtitle: c.subtitle, image };
}

// GET all 4 categories (public) — always returns exactly 4 entries (men,
// women, unisex, new), even if some haven't been customized in the admin
// panel yet, by filling in sensible defaults for any that don't exist yet.
app.get('/api/categories', async (req, res) => {
  try {
    const existing = await Category.find().lean();
    const bySlug = Object.fromEntries(existing.map(c => [c.slug, c]));
    const result = CATEGORY_SLUGS.map(slug =>
      bySlug[slug]
        ? normalizeCategory(bySlug[slug])
        : { slug, title: CATEGORY_DEFAULTS[slug].title, subtitle: CATEGORY_DEFAULTS[slug].subtitle, image: null }
    );
    res.set('Cache-Control', 'public, max-age=30');
    res.json(result);
  } catch (err) {
...
// Same data for the admin panel (no caching, so edits show immediately)
app.get('/api/categories/all', adminAuth, async (req, res) => {
  try {
    const existing = await Category.find().lean();
    const bySlug = Object.fromEntries(existing.map(c => [c.slug, c]));
    const result = CATEGORY_SLUGS.map(slug =>
...
// UPDATE one category by slug (admin) — upserts, so the first edit creates
// the document and later edits just update it.
app.put('/api/categories/:slug', adminAuth, async (req, res) => {
  try {
    const { slug } = req.params;
    if (!CATEGORY_SLUGS.includes(slug)) {
      return res.status(400).json({ error: `Invalid category slug. Must be one of: ${CATEGORY_SLUGS.join(', ')}` });
    }
    ...
    res.json({ message: 'Category updated ✅', category: normalizeCategory(updated) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
